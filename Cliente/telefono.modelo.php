...
class TelefonoModel
{
	...
	public function Obtener($id)
	{
		try 
		{
			...
			$stm = $this->pdo->prepare("SELECT id_telefono FROM cliente_has_telefono  where cliente_has_telefono.id_cliente= ? ");
	 ...
	 public function Obtener1($id)
	{
		try 
		{
			...
			$stm = $this->pdo->prepare("SELECT id_telefono FROM profesor_has_telefono where profesor_has_domicilio.id_profesor= ? ");
			$stm->execute(array($id));
			$r= $stm->fetch(PDO::FETCH_OBJ);
