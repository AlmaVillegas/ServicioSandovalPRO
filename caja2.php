        ...
        if(!isset($_SESSION["username"]))
        {
         ...
         if(!empty($_POST['tmp_cantidad']) and !empty($_POST['tmp_nombre']) and !empty($_POST['tmp_valor']))
          {  $tmp_cantidad=$_POST['tmp_cantidad'];
             ...
             $sql="INSERT INTO caja_tmp2 (cod, nom, compra, cant, importe, exitencia, usu) VALUES  ('0000','$tmp_nombre','$tmp_valor','$tmp_cantidad','$tmp_importe','$tmp_cantidad','$nombre_usu')";
             mysql_query($sql);
           }
?>
<!DOCTYPE html>
...
<table width="100%" border="0">
  <tr>
    <td width="10%" rowspan="2"><div class="control-group info">
        <form name="form1" method="post" action="">
          <label>
          <input type="text" autofocus class="input-xlarge" id="codigo" name="codigo" list="characters" placeholder="Codigo del Producto o Nombre del producto" autocomplete="off">
      ...
      <?php 
        ...
        {
          $codigo=$_POST['codigo'];
          $can=mysql_query("SELECT * FROM caja_tmp2 where cod='$codigo' or nom='$codigo'");
			if($dato=mysql_fetch_array($can))
        {
          $acant=$dato['cant']+1;	
          $dcodigo=$dato['cod'];	
          $acompra=$dato['compra']*$acant;
				  $sql="Update caja_tmp2 Set importe='$acompra', cant='$acant' Where cod='$dcodigo'";
          mysql_query($sql);	
			  }else{
          ...
          if($datos=mysql_fetch_array($cans))
          {
            ...
            $usu=$nombre_usu;
            $sql="INSERT INTO caja_tmp2 (cod, nom, compra, cant, importe, existencia, usu) VALUES ('$cod','$nom','$compra','$cant','$importe','$existencia','$usu')";
            mysql_query($sql);
          }else{
            echo '<div class="alert alert-error" align="center">
                  <button type="button" class="close" data-dismiss="alert">×</button>
                  <strong>Producto no encontrado en la base de datos<br><a href="#mycrear" role="button" class="btn btn-success" data-toggle="modal">Crear Nuevo Producto </a>
                  </strong>
                  </div>';
          }
        }
    ...
    <!--<td width="30%" rowspan="2">
    	<!--<pre><center>Nombre del Encargado : <?php echo $nombre_usu; ?></center></pre>
        <div align="right">
    ...
    <!--<td width="46%" rowspan="2">
       <div align="right"> 
            <button type="button" class="<?php echo $vboton; ?>" onClick="window.location='GraficaCompras.php'">Compras Realizadas</button>
        </div>
    </td>-->
  </tr>
  <tr>
...
<?php
	$can=mysql_query("SELECT * FROM caja_tmp2 where usu='$usu'");
  if($dato=mysql_fetch_array($can))
    {
...
<table width="00%" border="0" class="table table-hover">
  ...
  <tr>
    ...
    <td>
      <div align="right"><a href="caja2.php?id=<?php echo $dato['cod'].'&ddes='.$_SESSION['ddes']; ?>">$ <?php echo number_format($dato['compra'],2,",","."); ?></a>
      </div>
    </td>
    <td><center><a href="caja2.php?idd=<?php echo $dato['cod'].'&ddes='.$_SESSION['ddes']; ?>"><?php echo $dato['cant']; ?></a></center></td>
    <td bgcolor="#D9EDF7"><div align="right">$ <?php echo number_format($dato['importe'],2,",","."); ?></div></td>
    <td>
      <center><?php 
	       if(($dato['existencia']+$dato['cant'])>0)
          {
		        echo $dato['existencia']+$dato['cant'];
	        }else{
		        echo 0;
	       }
	    ?></center></td>
    <td> 
    <button type="button" class="btn btn-danger" onClick="window.location='php_eliminar_caja2.php?id=<?php echo $dato['cod']; ?>'"><i class="icon-minus-sign"></i> Remover</button>
    </td>
  <?php } ?>
...
?>
<form name="form2" method="get" action="php_caja_act2.php">
...
<form name="form2" method="post" action="php_caja2.php">
  <input type="hidden" name="xcodigo" id="xcodigo" value="<?php echo $_GET['idd'] ?>">
  Cantidad: <input type="text" name="cantidad" id="cantidad" value="0" pattern="^[0-9]+" autocomplete="off">
...
<table width="100%" border="0">
  <tr>
    ...
    <td width="36%">
    	<div align="right">
    	  <pre style="font-size:24px">Neto: <?php $can=mysql_query("SELECT SUM(importe) as neto FROM caja_tmp2 where usu='$nombre_usu'");
	  		if($dato=mysql_fetch_array($can))
          {
  ...
  <tr>
    ...
    <div class="well" style="max-width: 400px; margin: 0 auto 10px;">
        <div align="center">
       <a href="#myContado" role="button" class="btn btn-success" data-toggle="modal"><i class=" icon-shopping-cart"></i> Compra</a>
       <button type="button" class="btn btn-danger" onClick="window.location='php_eliminar_todo2.php'".php'">Cancelar</button>
        </div>
    </div>
    <?php } ?>
    </td>
...
<div id="myContado" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    ...
    <h3 id="myModalLabel">Cantidad Comprada</h3>
  </div>
  <div class="modal-body">
    	<p align="center" class="text-info"><strong>Total Comprado</strong></p>
        ...
        <div align="center">
          <form id="form1" name="contado" method="POST" action="contado_credito2.php">
                <!--<label for="ccpago">Dinero Recibido</label>-->
                <input type="hidden" name="tpagar" id="tpagar" value="<?php echo $_SESSION['neto']; ?>">
                <label><strong>N°Folio de la compra: </strong></label>
                <input type="text" id="folio" name="folio" required></input>
                <label><strong>Proveedor:</strong></label>
                <select id="id_proveedor" name="id_proveedor"  class="form-control" style="width:40%;">
                ...
                <textarea type="text" id="nota" name="nota" required></textarea> 
                <br>
                <br>
                <input type="submit" class="btn btn-success" name="button" id="button" value="Compra Realizada" />
          </form>
        </div> 
  </div>
